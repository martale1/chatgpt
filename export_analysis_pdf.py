...
7. Assegna una sola azione prevalente: HOLD, HOLD_OR_TRAILING_STOP, REDUCE, REVIEW_POSITION oppure EXIT_CANDIDATE. Non usare BUY_CANDIDATE, DO_NOT_BUY o suggerimenti da watchlist.
8. Fornisci condizioni osservabili che farebbero confermare o cambiare l'azione: chiusura giornaliera, supporto perso, breakout, volumi, notizie avverse o miglioramento fondamentale.
...
- Crea una tabella portafoglio con: ticker, quantita, prezzo di carico, prezzo corrente, valore corrente, P/L EUR, P/L %, sentiment news, trend, rischio, azione, priorita, livello di protezione e livello obiettivo.
...
- `securities`: esattamente una voce per ciascuna posizione e nello stesso ordine. Ogni voce deve contenere `ticker`, `company`, `source_page`, `position`, `news_research`, `news_summary`, `sentiment_analysis`, `recent_news_7d`, `older_relevant_news`, `analyst_target`, `technical`, `portfolio_assessment`, `operational_plan`, `management_rules`.
- `position`: `status` = `OPEN`, `quantity`, `entry_price`, `purchase_date`, `fees` copiati dai dati forniti; `current_price`, `price_timestamp`, `price_source`, `market_value`, `unrealized_gain_eur`, `unrealized_gain_pct` calcolati oppure null se non verificabili.
...
- Stop loss e take profit devono derivare dai grafici, dalla volatilita e dal prezzo di carico, non da percentuali arbitrarie. Se non sono determinabili con sufficiente affidabilita usa null e `plan_status` = `REVIEW_REQUIRED` o `NOT_AVAILABLE`. Non presentare mai questi livelli come ordini da eseguire automaticamente.
